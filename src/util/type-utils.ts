      ...
      [K in keyof DB[T]]: DB[T][K] extends any[] | undefined
        ?
            ...
            | (DB[T][K] extends (infer U)[] | undefined
                ...
                : never)
        : NonUndefined<DB[T][K]> extends object
          ? AnyArrayPropertyPath<{ _: NonUndefined<DB[T][K]> }, '_', Decrement[Depth], Path extends '' ? `${K & string}` : `${Path}.${K & string}`>
          : never;
    }[keyof DB[T]];

// Helper type to extract the item type of an array property
export type ExtractArrayItemType<DB, T extends keyof DB, P extends string> = P extends `${infer Key}[${number}].${infer Rest}`
...
export type ExtractArrayItemTypeWithTable<DB, TB extends keyof DB, P extends string> = P extends `${infer Table}.${infer Rest}`
  ? Table extends TB
...
export type AnyObjectPropertyPath<DB, T extends keyof DB, Depth extends number = 5, Path extends string = ''> = Depth extends 0
  ? never
  : {
...
export type AnyObjectPropertyPathWithTable<DB, TB extends keyof any> = {
  [T in TB]: T extends keyof DB ? `${T & string}.${AnyObjectPropertyPath<DB, T> & string}` : never;
}[TB];

/**
 * Just like {@link AnyObjectPropertyPathWithTable} but with a ` as <string>` suffix.
 */
export type AnyAliasedObjectPropertyPathWithTable<DB, TB extends keyof DB> = `${AnyObjectPropertyPathWithTable<DB, TB>} as ${string}`;

export type AnyMatchingObjectPropertyPath<DB, T extends keyof DB, M, Depth extends number = 5, Path extends string = ''> = Depth extends 0
  ? never
  : {
      [K in keyof DB[T]]: NonUndefined<DB[T][K]> extends object
        ? NonUndefined<DB[T][K]> extends any[] | undefined
 ...
 */
export type AnyAliasedArrayPropertyPathWithTable<DB, TB extends keyof DB> = `${AnyArrayPropertyPathWithTable<DB, TB>} as ${string}`;
 ...
 */
export type AnyAliasedPropertyPath<DB, TB extends keyof DB> = `${AnyPropertyPath<DB, TB>} as ${string}`;
...
type DirectExtract<T, P extends string> = T extends any ? (P extends keyof T ? T[P] : never) : never;
...
async function JoinTest(db: Kysely<Database>) {
  test('select all tests', async () => {
    ...
    const result2 = db.selectFrom('orders.customer.phoneNumbers as p').selectAll().compile();

    const result3 = db.selectFrom('orders.items[0] as firstItem').selectAll().compile();

    const result4 = db.selectFrom('orders.customer.phoneNumbers as p').selectAll().compile();

    const result5 = db.selectFrom('orders.items[0] as firstItem').where('firstItem.taxes[0].type', '=', 'VAT').selectAll().compile();
  });

  test('alias test', async () => {
    const result = db.selectFrom('families as f').select(['f.id', 'f.address.city']).where('f.address.city', '=', 'Dallas').compile();
  });

  test('deep select test', async () => {
    const result1 = db.selectFrom('nested as n').select('n.level1.level2.level3IsAnArray[0].level4.name4').compile();

    const result2 = db.selectFrom('nested as n').select('n.level1.level2.level3IsAnArray[0].colors').compile();
  });

  test('order by clause test', async () => {
    const result = db.selectFrom('families as f').select(['f.id', 'f.address.city']).orderBy('f.address.city', 'asc').compile();
  });
  ...
  test('in clause test', async () => {
    ...
    const result = db.selectFrom('families as f').select(['f.id', 'f.lastName', 'f.address']).where('f.address.state', 'in', states).compile();
  });
  ...
  test('select arithmetic expression test', async () => {
    const result = db
      ...
      .join('i in o.items')
      .select(sql<number>`(i.quantity * i.unitPrice) * (1 - i.discount)`.as('amount'))
      .compile();
  });
  ...
  test('parameterized where test', async () => {
    const city = 'Dallas',
      state = 'TX';
    const result = db.selectFrom('families as f').selectAll().where('f.address.city', '=', city).where('f.address.state', '=', state).compile();
  });

  test('limit clause test', async () => {
    const result = db.selectFrom('families as f').selectAll().orderBy('f.address.city').limit(10).compile();
  });
  ...
  test('group by having simulation test', async () => {
    const subquery = db
      ...
      .join('i in o.items')
      .groupBy('i.category')
      .select(eb => ['i.category', eb.fn.sum<number>('i.amount').as('totalAmount')]);
  ...
  test('group by with multiple joins test', async () => {
    const result = db
      ...
      .join('t in o.tags')
      .groupBy('i.category')
      .select(eb => ['i.category', eb.fn.sum<number>('i.amount').as('totalAmount'), 't.name'])
      .compile();
  });
  ...
  test('group by multiple columns test', async () => {
    const result = db
      ...
      .select(eb => ['i.category', 'i.productId', eb.fn.sum<number>('i.amount').as('totalAmount')])
      .compile();
  });
  ...
  test('join with group by and where clause test', async () => {
    const result = db
      ...
      .where('o.total', '>', 100)
      .groupBy('i.category')
      .select(eb => ['i.category', eb.fn.count<number>('i.productId').as('productCount')])
      .compile();
  });
  ...
  test('subquery test', async () => {
    ...
    const result = db.selectFrom(subquery.as('grouped')).select(['grouped.familyCount']).compile();
  });

  test('discriminator and logical deletion test', async () => {
    const result = db
      .selectFrom('families as f')
      .where(({ eb, or }) => or([eb('f.address.city', '=', 'Dallas'), eb('f.address.city', '=', 'Austin')]))
  ...
  test('select from nested object test', async () => {
    const result = db.selectFrom('orders.items[0] as i').select('i.taxes').orderBy('i.taxes[0].type').compile();

    const result1 = db.selectFrom('orders.items[0].taxes[3] as t').select('t.type').compile();
  });

  test('select from nested object test', async () => {
    const result = db.selectFrom('orders.customer as c').select('c.firstName').compile();
  });

  test('select indexed-access off optional array test', async () => {
    const result1 = db.selectFrom('orders as o').select('o.items[0].quantity').compile();

    const result2 = db
      .selectFrom('orders as o')
      .select(eb => eb.fn.intBitOr('o.total', 'o.items[0].quantity').as('orValues'))
      .compile();
  });

  test('join test with deep path select', async () => {
    const result1 = db.selectFrom('orders as o').join('i in o.items').select('i.taxes[5] as t').compile();

    const result2 = db.selectFrom('orders as o').join('i in o.items').select('i.taxes as taxes').compile();

    const result3 = db.selectFrom('orders as o').join('i in o.items').select('i.taxes[0].type as taxType').compile();

    const result4 = db.selectFrom('orders as o').join('i in o.items').select('i.taxes[0].type').compile();
    ...
    const result6 = db.selectFrom('orders as o').join('i in o.items').select('i.productId').compile();
  });

  // Diagnostic type to inspect SelectExpression for the joined query
  type DiagnosticSelectExpression = SelectExpression<Database & { o: Order } & { i: ArrayItemType<Order['items']> }, 'o' | 'i'>;

  // Diagnostic type to check assignability
  type DiagnosticIsAssignable = 'i.taxes[0].type' extends DiagnosticSelectExpression ? true : false; // Should be DiagnosticIsAssignable = true
  type DiagnosticIsAliasedAssignable = 'i.taxes[0].type as taxType' extends DiagnosticSelectExpression
