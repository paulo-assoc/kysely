...
import { OperationNodeSource, isOperationNodeSource } from '../operation-node/operation-node-source.js';
import { OperatorNode, ComparisonOperator, BinaryOperator, Operator, OPERATORS } from '../operation-node/operator-node.js';
import {
...
import { parseValueExpression, parseValueExpressionOrList, ValueExpression, ValueExpressionOrList } from './value-parser.js';
...
export type FilterObject<DB, TB extends keyof DB> = {
  [R in AnyReference<DB, TB>]?: ValueExpressionOrList<DB, TB, SelectType<ExtractTypeFromStringReference<DB, TB, R>>>;
};

export function parseValueBinaryOperationOrExpression(args: any[]): OperationNode {
  if (args.length === 3) {
...
export function parseFilterObject(obj: Readonly<FilterObject<any, any>>, combinator: 'and' | 'or'): OperationNode {
...
function isIsOperator(operator: BinaryOperatorExpression): operator is 'is' | 'is not' {
...
function toOperationNode(nodeOrSource: OperationNode | OperationNodeSource): OperationNode {
