...
import { createJoinBuilder, createSubqueryJoinBuilder } from './parse-utils.js';
import { From, FromTables, TableExpression, parseJoinExpression, parseTableExpression } from './table-parser.js';
...
export function parseJoin(joinType: JoinType, args: any[]): JoinNode {
  if (args.length === 3) {
    ...
    if (joinType === 'Join' && typeof args[1] === 'function') {
      return parseSubqueryJoin(joinType, args[0], args[1]);
    }
...
function parseCallbackJoin(joinType: JoinType, from: TableExpression<any, any>, callback: JoinCallbackExpression<any, any, any>): JoinNode {
  return callback(createJoinBuilder(joinType, from)).toOperationNode();
}

function parseSingleOnJoin(joinType: JoinType, from: TableExpression<any, any>, lhsColumn: string, rhsColumn: string): JoinNode {
  return JoinNode.createWithOn(joinType, parseTableExpression(from), parseReferentialBinaryOperation(lhsColumn, '=', rhsColumn));
}

function parseOnlessJoin(joinType: JoinType, from: TableExpression<any, any>): JoinNode {
...
function parseSubqueryJoin(joinType: JoinType, from: string, callback: (sq: any) => { toOperationNode(): any }): JoinNode {
  const sq = createSubqueryJoinBuilder(from);
  const aliasedResult = callback(sq);
  return JoinNode.create(joinType, aliasedResult.toOperationNode());
}
