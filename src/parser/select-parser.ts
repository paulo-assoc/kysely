...
import { parseAliasedStringReference, parseStringReference } from './reference-parser.js';
import { DynamicReferenceBuilder, isDynamicReferenceBuilder } from '../dynamic/dynamic-reference-builder.js';
import { AliasedExpressionOrFactory, parseAliasedExpression } from './expression-parser.js';
import { SelectType } from '../util/column-type.js';
import { parseTable } from './table-parser.js';
import { AliasedExpression, isExpression } from '../expression/expression.js';
...
type FlattenSelectExpression<SE> = SE extends DynamicReferenceBuilder<infer RA> ? { [R in RA]: DynamicReferenceBuilder<R> }[RA] : SE;
...
type ExtractAliasFromStringSelectExpression<SE extends string> = SE extends `${string} as ${infer A}`
  ? A
...
export type ExtractTypeFromSelectExpression<DB, TB extends keyof DB, SE> = SE extends string
...
export type ExtractTypeFromStringSelectExpression<DB, TB extends keyof DB, SE extends string> = SE extends `${infer T}.${infer P} as ${string}`
  ? T extends TB
...
export function parseSelectArg(selection: SelectArg<any, any, SelectExpression<any, any>>): SelectionNode[] {
...
function parseSelectExpression(selection: SelectExpression<any, any>): SelectionNode {
...
export function parseSelectValueExpression(
  selection: SelectExpression<any, any> | import('../expression/expression.js').Expression<any>
): SelectionNode[] {
  if (isString(selection)) {
  ...
  } else if (isExpression(selection)) {
    return [SelectionNode.createValueSelection(selection.toOperationNode())];
  } else {
    return [SelectionNode.createValueSelection(parseAliasedExpression(selection))];
  }
}
