...
import { OrderByNode, OrderByRankNode } from '../operation-node/order-by-node.js';
...
import { SelectionNode, ValueSelectionNode } from '../operation-node/selection-node.js';
...
import { SelectModifier, SelectModifierNode } from '../operation-node/select-modifier-node.js';
...
export class DefaultQueryCompiler extends OperationNodeVisitor implements QueryCompiler {
  #sql = '';
  #parameters: unknown[] = [];
  ...
  protected compileList(nodes: ReadonlyArray<OperationNode>, separator = ', '): void {
  ...
  protected buildDeferrable(node: { deferrable?: boolean; initiallyDeferred?: boolean }): void {
  ...
  protected sortSelectModifiers(arr: SelectModifierNode[]): ReadonlyArray<SelectModifierNode> {
    arr.sort((left, right) =>
      left.modifier && right.modifier ? SELECT_MODIFIER_PRIORITY[left.modifier] - SELECT_MODIFIER_PRIORITY[right.modifier] : 1
    );

    return freeze(arr);
  }

  protected compileColumnAlterations(columnAlterations: readonly AlterTableColumnAlterationNode[]) {
...
const SELECT_MODIFIER_PRIORITY: Readonly<Record<SelectModifier, number>> = freeze({
  ForKeyShare: 1,
  ForNoKeyUpdate: 1,
  ForUpdate: 1,
  ForShare: 1,
