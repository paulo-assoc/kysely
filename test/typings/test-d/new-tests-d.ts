...
async function JoinTest(db: Kysely<Database>) {
  test('select all tests', async () => {
    ...
    const result1b = await db.selectFrom('persons as p').selectValue('p').compile();
  ...
  test('arrayContains function tests', async () => {
    const result1 = db
      .selectFrom('families as f')
      .selectAll()
      .where(eb => eb.fn.arrayContains('f.children[0].pets', { age: 5 })) // full match
      .compile();

    const result2 = db
      .selectFrom('orders.tags as t')
      .selectAll()
      .where(eb => eb.fn.arrayContains('t', { name: 'electronics' }, false)) // full match
      .compile();

    const result3 = db
      .selectFrom('orders.tags as t')
      .selectAll()
      .where(eb => eb.fn.arrayContains('t', { name: 'electronics' }, true)) // partial match
      .compile();
    ...
    const createColorsPredicate = (
      ...
      return eb.or(colors.map(color => eb.fn.arrayContains(`o.colors`, color)));
    };
  ...
  test('join test with deep path select', async () => {
    const case1 = db.selectFrom('orders as o').join('i in o.items').select('i.taxes').compile();

    const case2 = db.selectFrom('orders as o').join('i in o.items').select('i.taxes[0].type').compile();
  });

  type T1 = AnyPropertyPath<Database, 'orders2'>;
  type T2 = AnyPropertyPathWithTable<Database, 'orders2'>;

  type AnyAliasedArray = AnyAliasedArrayPropertyPathWithTable<Database, 'orders'>;

  type AnyAliasedObjects = AnyAliasedObjectPropertyPathWithTable<Database, 'nested'>;

  type AnyAliasedObjects2 = AnyAliasedObjectPropertyPathWithTable<Database, 'orders'>;
}
